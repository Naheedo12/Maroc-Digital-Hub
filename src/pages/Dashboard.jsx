...
function Dashboard() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const [startupsData, eventsData] = await Promise.all([
          startupsAPI.getAll(),
          eventsAPI.getAll(),
        ])
        ...
        dispatch(fetchStartupsFailure(err.message))
  ...
  const totalParticipants = events.reduce(
    (total, e) => total + e.participants.length,
    0
  )
  ...
  const sectorData = sectors.map((sector) => {
    ...
    const percentage =
      startups.length > 0
        ? Math.round((count / startups.length) * 100)
        : 0
    return {
      sector,
      count,
    ...
    <div className="min-h-screen bg-[#fef5f1] py-12 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-12">
          <h1 className="text-4xl font-bold text-[#0d7377]">
            Tableau de Bord Administrateur
          </h1>
          <Link
            to="/"
            className="text-gray-600 hover:text-[#0d7377] flex items-center space-x-2 transition-colors"
          >
            <svg
              className="w-5 h-5"
              fill="none"
          ...
          {stats.map((stat, i) => (
            ...
            >
              <div className="flex items-center space-x-4 mb-2">
                {stat.icon}
                <div className="text-4xl font-bold text-[#0d7377]">
                  {stat.value}
                </div>
              </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-8 mb-8">
          <h2 className="text-2xl font-bold text-[#0d7377] mb-8">
            Répartition des Startups par Secteur
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            <h3 className="text-xl font-bold text-[#0d7377] mb-4">
              Dernières Startups
            </h3>
            <div className="space-y-3">
              {startups.length > 0 ? (
